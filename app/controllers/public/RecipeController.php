	...
	public function getRecipes(){
		if(Auth::user()){
			...
			if($user->user_type == 'B2B'){
				$rData = MenuRecipes::orderBy(DB::raw('RAND()'))->where('selection_active', '=', 1)->take(18)
				...
				$rData = MenuRecipes::orderBy(DB::raw('RAND()'))->where('selection_active', '=', 1)->where('exclusive', '!=', 1)->take(18)
					->with(array('MenuCategories' => function($query)
					{
						$query->where('menu_categories.active', '=', 1);
					}))
		...
		$aeData = MenuRecipes::where('selection_active', '=', 1)
		...
		// if($user->user_type != 'B2B'){
			$exrData = MenuRecipes::orderBy(DB::raw('RAND()'))->where('selection_active', '=', 1)->where('exclusive', '=', 1)
				->with(array('MenuCategories' => function($query)
				{
					$query->where('menu_categories.active', '=', 1);
				}))
